...
public class AvroSchemaMapper implements Serializable{
	...
	public GenericRecord map(List<CanInfoBean> canInfoBeanList) {
		GenericRecord record = new GenericData.Record(getSchema());
		record.put("canId", String.valueOf(canInfoBeanList.get(0).getCanId()));
		record.put("canTime", canInfoBeanList.get(0).getCanTime());
        return record;
	}
}
